function [BER,BER_analytic,Gammab]=SER_BER_16QAM(Nsymb,BER_t,display_mode)
...
for i=1:length(SNRb)
    ...
    if display_mode==1
        display_constellation(real(r),imag(r),16)
        colorbar
    else
     ...
     line([2,2],[-5,5],'LineWidth',3,'Color',[0 0 0]);
      title (strcat(['16QAM  with SNR per bit =',num2str(10*log10(Gammab(i))),'dB ans BER =', num2str(BER(i))]),'fontsize',14)
